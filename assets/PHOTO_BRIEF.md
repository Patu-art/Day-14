# Strange Brew: photos for a client-approved release

The current independent website concept uses five photographs supplied by the requester, showing the venue and its customers. They are **not independently verified as licensed for commercial or official use**. Obtain permission from the rights holder, the venue and any identifiable people as appropriate before official publication.

Request venue-owned, licensed photographs with permission and consent from identifiable guests where relevant:
- Warm wide shot of the real red seating area, 1600px wide or greater.
- Front patio/exterior and recognisable signage, 1400px wide or greater.
- Bar/cask/keg counter and actual current drinks, 1200px wide or greater.
- Rear live-music/open-decks area without misleading dates or performers.

Use optimized local WebP/JPEG in assets/ after obtaining rights. Add descriptive alt text and layout-aware crops; do not use third-party Instagram/Google hotlinks or scrape their images without permission.


## Five supplied photos already in the project
- hero-exterior-evening.webp — real bar exterior (hero).
- vinyl-records-interior.webp — browsing records indoors (events/music).
- drinks-pint-closeup.webp — beer and taps (drinks).
- crowd-atmosphere.webp — busy room (the place).
- front-exterior-people.webp — front of the venue (visit).

These were supplied by the requester and optimized to WebP for this demo. The obsolete stock atmosphere photograph was removed. Obtain venue approval and any necessary image/guest permissions before an official client launch.
